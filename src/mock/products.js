export const products = [
    {
        id: 1,
        name: 'Remera',
        price: 800,
        stock: 4,
        img: 'https://res.cloudinary.com/sabietcheverry99/image/upload/v1657483174/remera_basica.jpeg_bil2rk.jpg',
        category: 'remeras',
        description:
            'Lorem ipsum dolor sit amet consectetur adipisicing elit. Deserunt iusto magni quaerat nisi quisquam? Error, earum aspernatur tenetur sint cum in quibusdam eos quae velit ratione maxime! Error, earum repellendus.',
    },
    {
        id: 2,
        name: 'Jean',
        price: 1800,
        stock: 3,
        img: 'https://res.cloudinary.com/sabietcheverry99/image/upload/v1657483165/jean_negro_fg7hau.jpg',
        category: 'pantalon',
        description:
            'Lorem ipsum dolor sit amet consectetur adipisicing elit. Deserunt iusto magni quaerat nisi quisquam? Error, earum aspernatur tenetur sint cum in quibusdam eos quae velit ratione maxime! Error, earum repellendus.',
    },
    {
        id: 3,
        name: 'Campera',
        price: 2100,
        stock: 5,
        img: 'https://res.cloudinary.com/sabietcheverry99/image/upload/v1657483156/campera_cuero_qfop7v.jpg',
        category: 'camperas',
        description:
            'Lorem ipsum dolor sit amet consectetur adipisicing elit. Deserunt iusto magni quaerat nisi quisquam? Error, earum aspernatur tenetur sint cum in quibusdam eos quae velit ratione maxime! Error, earum repellendus.',
    },
   
];